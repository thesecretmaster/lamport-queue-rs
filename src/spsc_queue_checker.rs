...
mod thread_state {
    ...
    impl<T: 'static + Clone + Send> ThreadState<T> {
        ...
        pub fn run_thread<F: 'static + FnOnce(T) -> ThreadRet<T> + Send>(&mut self, f: F) -> Result<(), &'static str> {
            ...
            } else { None }.ok_or("Only one reciever can be started for each checker")?;
...
impl<T: 'static + Default + Copy + PartialEq + Send> SPSCChecker<T> {
    ...
    // Creates an equivilent but fresh SPSCChecker. Specifically, the list elements
    ...
    pub fn check(&mut self) -> bool {
        ...
        let (send_list, start_time) = self.send_thread_state.join().expect("Cannot check if a sender thread hasn't been started");
        let (recieve_list, end_time) = self.recieve_thread_state.join().expect("Cannot check if a reciever thread hasn't been started");
